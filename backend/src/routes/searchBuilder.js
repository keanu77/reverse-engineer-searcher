...
const sanitizeLLMConfig = (llmConfig = {}) => {
  ...
  if (provider && !ALLOWED_PROVIDERS.includes(provider) && provider !== 'custom') {
  ...
  // 驗證長度限制
  if (llmConfig.apiKey && llmConfig.apiKey.length > VALIDATION_LIMITS.maxApiKeyLength) {
    throw new Error('API Key 長度超過限制');
  }
  if (llmConfig.baseURL && llmConfig.baseURL.length > VALIDATION_LIMITS.maxBaseURLLength) {
    throw new Error('Base URL 長度超過限制');
  }
  if (llmConfig.model && llmConfig.model.length > VALIDATION_LIMITS.maxModelLength) {
...
router.post('/generate-blog', async (req, res) => {
  try {
    ...
    // 驗證 query_string 長度
    if (typeof query_string !== 'string' || query_string.length > VALIDATION_LIMITS.maxQueryStringLength) {
      return res.status(400).json({
        error: 'Invalid input',
    ...
    // 驗證 topic 長度
    if (topic && (typeof topic !== 'string' || topic.length > VALIDATION_LIMITS.maxTopicLength)) {
      return res.status(400).json({
        error: 'Invalid input',
    ...
    const allPmids = [...new Set([...cleanedGoldPmids, ...searchResult.pmids])].slice(0, 15);
